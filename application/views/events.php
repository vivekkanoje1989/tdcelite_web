							...
							<div class="col-sm-12 col-xs-12">
								<div class="list_block related_post_sec">
								  <div class="upcoming_events">
									...
									<ul id="get_events_upcoming">
									  <!-- <li class="related_post_sec single_post">
										<span class="date-wrapper">
										  <span class="date"><span>< ?php echo date("d");?></span> < ?php echo date("M");?>&nbsp;2017</span>
										</span>
										<div class="rel_right">
									...
									<a href="<?php echo base_url();?>index.php/tdcelite/go_events_form" class="btn btn-default btn-block commonBtn">ADD Events</a>
								  </div>
							  </div>
						</div>
...
<script >
    	$(document).ready(function() {
			...
			toastr.info('Showing Upcoming Events', 'Information', {timeOut: 5000})		
      	});    	
    </script>
<script>
...
<script>
     $(document).ready(function(){
		...
		monthsArray[11] = 'Dec';
     	$.ajax({
            type: "GET",
            url: '<?php echo base_url();?>index.php/tdcelite/event_onload_fmToday?onload= true',
            success: function(data){            
               
                //alert("data:" + data);    
                var response = JSON.parse(data);
                $("#get_events_upcoming .rm").remove();
                $.each(response, function(i, item) {
                	var g_date = new Date(item.event_date);
                	var d = g_date.getDate(); 	var m = monthsArray[g_date.getMonth()];	var y = g_date.getFullYear();
                	var time = null;
                	var hours = new Date(""+m+" "+d+" "+y+" "+item.event_time+"").getHours();
                	var minute = new Date(""+m+" "+d+" "+y+" "+item.event_time+"").getMinutes();
                	var mid='AM';
				   	if (hours < 12) {
				   		hours = hours;
				   		mid='AM';
				    ...
				    if (hours.toString().length == 1) { var h = "0"+hours; }else{ var h = hours;}
				    if (minute.toString().length == 1) { var mt = "0"+minute; }else{ var mt = minute;}
				    time = ''+h+':'+mt+' '+mid+'';
				      var $li = $('<li class="rm">').append(                                            
				        $("<li class='related_post_sec single_post'><span class='date-wrapper'><span class='date'><span>"+d+"</span>"+m+" &nbsp;"+y+"</span></span><div class='rel_right'><h4><a href=''>"+item.event+"</a></h4><div class='meta'><span class='place'><i class='fa fa-map-marker'></i>"+item.location+"</span><span class='event-time'><i class='fa fa-clock-o'></i>"+time+"</span></div></div></li>")
				        ).appendTo('#get_events_upcoming');				                      
                });            
            }
        });
    });
